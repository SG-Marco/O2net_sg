...
class DeformableDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor, need_prop=None):
        ...
        if need_prop is not None:
            ...
            for i in range(self.num_feature_levels):
                src = srcs[i]
                domain_out, domain_label = self.discriminator(src, need_prop)
        ...
        if need_prop is not None:
            return out, domain_outs, domain_labels, masks, hs
        else:
            return out
...
def DA_build(args):
    ...
    dataset2numcls = {
        'city2foggy_source': 9, 'city2foggy': 9,
    }
